...
class Repository {
  constructor(repoName, owner, url, token) {
    this.repo = repoName;
    this.owner = owner;
  ...
  async createRemoteRepository(description = null, org = null) {
    try {
  ...
  async getHooks() {
    const res = await this.axios.get(`/repos/${this.owner}/${this.repo}/hooks`);
    return res.data;
  }
